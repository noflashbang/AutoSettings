#ifndef AUTO_POINTER_BASE_TYPE
#define AUTO_POINTER_BASE_TYPE

template <typename T>
class AutoPointerBaseType
{
public:
	void Build()
	{
	...
	AutoPointerBaseType(AutoPointerBaseType<T> &ref)
	{
		if (ref.m_Ptr == NULL)
			ref.Build();
	...
	AutoPointerBaseType<T>& operator= (AutoPointerBaseType<T> &ref)
	{
		if (this != &ref)
		{
			if (ref.m_Ptr == NULL)
